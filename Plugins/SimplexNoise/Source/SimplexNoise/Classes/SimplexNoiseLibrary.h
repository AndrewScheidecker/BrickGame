...
public:
	...
	static SIMPLEXNOISE_API float Sample(float X);

	// Samples a 2D simplex noise function, returning a value between 0-1.
	UFUNCTION(BlueprintCallable,Category=Noise)
	static SIMPLEXNOISE_API float Sample2D(float X,float Y);

	// Samples a 3D simplex noise function, returning a value between 0-1.
	UFUNCTION(BlueprintCallable,Category=Noise)
	static SIMPLEXNOISE_API float Sample3D(float X,float Y,float Z);
};
